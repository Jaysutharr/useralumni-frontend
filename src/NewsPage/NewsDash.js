...
const NewsDash = () => {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        ...
        const sortedNews = (response.data.news || []).sort((a, b) =>
          new Date(b.publishedAt) - new Date(a.publishedAt)
        );

        console.log('🔢 Sorted news count:', sortedNews.length);
        console.log('📰 First news item:', sortedNews[0]);

        setNewsData(sortedNews);
        setLoading(false);
        console.log('✅ News data set successfully');
      } catch (err) {
  ...
  const handleCreateNewsNavigation = () => {
    navigate('/create-news')
  }

  const handleEditNewsNavigation = (news) => {
    navigate('/edit-news', { state: { news } });
  }

  const handleReadNewsNavigation = (news) => {
    navigate('/news-read', { state: { news } });
  }

  // Get featured news (latest) and sidebar news (rest)
  const featuredNews = newsData.length > 0 ? newsData[0] : null;
  const sidebarNews = newsData.slice(1, 6); // Next 5 news items
    ...
    <div className="dashboard-container-newsdash">
      ...
      <header className="header1-newsdash">
        <div className="logo-section-newsdash">
          ...
          <h2 className="dashboard-text-newsdash">News</h2>
        </div>
        ...
        <div className="user-section-newsdash">
          <button
            onClick={handleCreateNewsNavigation}
            style={{
              ...
              boxShadow: '0 4px 15px rgba(88, 164, 176, 0.3)',
              marginRight: '1rem'
            }}
      ...
      <div className="content-container-newsdash">
        <div className="news-layout">
          ...
          <div className="news-main">
            <h3 className="news-title">Today’s Headline</h3>

            {loading ? (
              <div style={{ padding: '2rem', textAlign: 'center' }}>Loading...</div>
            ) : featuredNews ? (
              <div className="headline-card">
                <span className="tag">{featuredNews.category}</span>

                <h2>{featuredNews.title}</h2>

                <div className="headline-footer">
                  <div className="stats">
                    👁 {featuredNews.views || 0} &nbsp; 💬 {featuredNews.comments || 0} &nbsp; 🔗 {featuredNews.shares || 0}
                  </div>
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button
                      className="read-btn"
                      onClick={() => handleReadNewsNavigation(featuredNews)}
                    >
                      Read Now
                    </button>
                    <button
                      onClick={() => handleEditNewsNavigation(featuredNews)}
                      style={{
                        padding: '0.5rem 1rem',
                        background: 'white',
                        color: '#58a4b0',
          ...
          <div className="news-right">
            ...
            {loading ? (
              ...
              sidebarNews.map((news) => (
                <div className="recent-card" key={news._id}>
                  <div className="recent-img"></div>

                  <div className="recent-content">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.5rem' }}>
                      <p
                        style={{ cursor: "pointer", flex: 1, lineHeight: '1.4' }}
                        onClick={() => handleReadNewsNavigation(news)}
                      >
                        {news.title}
                      </p>
                      <button
                        onClick={() => handleEditNewsNavigation(news)}
                        style={{
                          width: '32px',
                          height: '32px',
                          padding: '0',
                      ...
                      >
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M11 4H4C3.46957 4 2.96086 4.21071 2.58579 4.58579C2.21071 4.96086 2 5.46957 2 6V20C2 20.5304 2.21071 21.0391 2.58579 21.4142C2.96086 21.7893 3.46957 22 4 22H18C18.5304 22 19.0391 21.7893 19.4142 21.4142C19.7893 21.0391 20 20.5304 20 20V13" stroke="#58a4b0" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                          <path d="M18.5 2.50001C18.8978 2.10219 19.4374 1.87869 20 1.87869C20.5626 1.87869 21.1022 2.10219 21.5 2.50001C21.8978 2.89784 22.1213 3.4374 22.1213 4.00001C22.1213 4.56262 21.8978 5.10219 21.5 5.50001L12 15L8 16L9 12L18.5 2.50001Z" stroke="#58a4b0" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                      </button>
                    </div>
                    <div className="recent-stats">
                      👁 {news.views || 0} &nbsp; 💬 {news.comments || 0} &nbsp; 🔗 {news.shares || 0}
                    </div>
                  </div>
                </div>
              ))
        ...
        >
          ...
          <ul className="sidebar-menu-newsdash">
            ...
            <li className="menu-item-newsdash" onClick={handleArticleNavigation}>
