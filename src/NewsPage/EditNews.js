...
import './CreateNews.css'; // Reuse CreateNews styles
...
const EditNews = () => {
    ...
    // Pre-fill form with existing news data
    useEffect(() => {
        if (news) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!news || !news._id) {
            setError('News ID is missing. Cannot update.');
            return;
        }
        ...
        try {
            ...
            if (err.response) {
                ...
                if (status === 400) {
                    // Validation error
                    setError(errorData.message || 'Validation error. Please check your input.');
                } else if (status === 404) {
                    // News not found
                    setError('News article not found. It may have been deleted.');
                } else if (status === 500) {
                    // Server error
                    setError('Server error. Please try again later.');
                } else {
                    setError(errorData.message || 'Failed to update news. Please try again.');
                }
            } else if (err.request) {
        ...
        return (
            <div className="create-news-container">
                <div style={{ padding: '2rem', textAlign: 'center' }}>
                    ...
                    <p>Please select a news article to edit.</p>
                    <button onClick={() => navigate('/news')} className="submit-button-news">
    ...
    return (
        <div className="create-news-container">
            <div className="create-news-header">
                ...
                <h1 className="create-news-title">Edit News Article</h1>
            </div>
